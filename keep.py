...
def stacked_bar(data,key):
 ...
 d_f.plot.barh(stacked=True, figsize=(9,4),ax=ax).legend(loc='upper right', bbox_to_anchor=(1, 0, 0.15, 1),prop=thai_font_prop)
...
if upload_file is not None:
 ...
 for key in list_bar_chart:
  ...
  z = st.sidebar.slider(key, 1, max(c.values()), 1, 1)  # Fixed the syntax error here
