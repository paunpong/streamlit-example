  ...
  if Type == 'แท่งต่อกัน':
   ...
   for topic_stack in list_num_keys:
    ...
    if num == 'ไม่แปลผล' and topic_stack not in moved_topics:
     ...
     # หาหัวข้อที่มีคำหน้าเหมือนกัน
     similar_topics = [topic for topic in list_num_keys if topic.split('[')[0] == topic_stack.split('[')[0]]
        
     # ย้ายทุกหัวข้อที่มีคำหน้าเหมือนกันไปยังส่วน "แปลผล"
     for similar_topic in similar_topics:
      dic_stackbar_str_choosen[similar_topic] = list_stack_str[similar_topic]
        
     # เพิ่มรายการของหัวข้อที่ถูกย้ายไปแล้ว
     moved_topics.extend(similar_topics)
